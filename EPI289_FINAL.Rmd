...
hivpoint <- read_csv("/Users/raoyiwen/Desktop/hivpoint.csv")
...
for (i in grid){
  ...
  #logitc regression model to estimate what psi is
...
fit.para1 <- glm(treatment ~ age + sex + as.factor(origin) + year + as.factor(mode) + as.factor(cd4_0_cat) + as.factor(rna_0_cat), data = hivpoint.c, family=binomial())
summary(fit.para1)

hivpoint.c$p.treatment1 <- predict(fit.para1, type="response")

#IPW for selection bias, non-stabilized already calculated (w.c)

#MSM
msm.valid1 <- geeglm(logrna ~ treatment + p.treatment1 + I(p.treatment1 * p.treatment1), data=hivpoint.c, weights=w.c, id=id,
                corstr="independence")
summary(msm.valid1)
...
```{r}
#??
#could use hivpoint and $censoring ==0
#OR
#use hivpoint.c (PS is done in hivpoint.c)

# create a dataset with 3 copies of each subject
hivpoint.c$interv <- -1 # 1st copy: equal to original one

interv0 <- hivpoint.c # 2nd copy: treatment set to 0, outcome to missing
interv0$interv <- 0
interv0$treatment<- 0
interv0$logrna <- NA

interv1 <- hivpoint.c # 3rd copy: treatment set to 1, outcome to missing
interv1$interv <- 1
interv1$treatment<- 1
interv1$logrna <- NA

onesample <- rbind(hivpoint.c, interv0, interv1) # combining datasets
...
# bootstrap
results <- boot(data=hivpoint.c, statistic=standardization, R=500)
...
#obtain standardized risk difference comparing had everyone been treated versus had no one been treated
a <- mean(onesample[which(onesample$interv==1),]$predicted_meanY2) -
mean(onesample[which(onesample$interv==0),]$predicted_meanY2)
